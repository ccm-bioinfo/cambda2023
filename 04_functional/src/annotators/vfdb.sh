...
# Usage:
# ./vfdb.sh [-i input_faa] [-o output_tsv] [-d database_dir]
...
if [[ ! -d "$db/vfdb/" ]]; then
  >&2 echo "ERROR: VFDB not found in $db"; exit 1
fi
...
echo $(date +"%D %T:") Annotation of $(basename ${inp%.*}) with VFDB started
...
# Run blast
blastp -num_threads 12 -query ${inp} -db ${db}/vfdb/db \
...
echo $(date +"%D %T:") Annotation of $(basename ${inp%.*}) with VFDB finished
