...
# Usage:  ./vfdb.py [-g] [input directory] [output directory]
...
with Pool(processes=math.ceil(os.cpu_count() / 8)) as pool:
    for i, count in enumerate(pool.imap_unordered(process_tsv, inp)):
        ...
        print(
            f"VFDB tabulation progress: {progress:.2f}%",
            file=sys.stderr, end="\r"
        )
        counts.append(count)

# Create output table and save
print(f"VFDB tabulation progress: Concatenating...", file=sys.stderr, end="\r")
...
print(f"VFDB tabulation progress: Saving...       ", file=sys.stderr, end="\r")
data.to_csv(out/"vfdb.tsv.gz", sep="\t", index=True, compression="gzip")
print(f"VFDB tabulation progress: Done!           ", file=sys.stderr)
