...
def scores(model, pred, true):
    ...
    pd.DataFrame(zip(['Accuracy', 'Balanced Accuracy', 'F1'], [acc, bal, f1])).to_csv(f'../results/test_results{level}_{model}.csv')

level = 1

# for level in range(1, 3):
data = pd.read_csv(f'../data/03-contingencies/02-mifaser/level{level}.tsv', sep = '\t', index_col = 0)
...
pd.DataFrame(clf1.cv_results_).to_csv(f'../results/RF_results{level}.csv')
pd.DataFrame(clf2.cv_results_).to_csv(f'../results/SVC_results{level}.csv')
pd.DataFrame(clf3.cv_results_).to_csv(f'../results/MLP_results{level}.csv')
pd.DataFrame(clf4.cv_results_).to_csv(f'../results/ET_results{level}.csv')
...
names = ['RandomForest', 'SVC', 'MLP', 'ExtraTrees']
preds = [pred1, pred2, pred3, pred4]
true_ = [y_test, y_test, y_test, y_test]

pd.DataFrame(zip(names, preds, true_), columns = ['model', 'predictions', 'true']).to_csv(f'../results/predictions_level{level}.tsv', sep = '\t')

scores('RandomForest', pred1, y_test)
scores('SVC', pred2, y_test)
scores('MLP', pred3, y_test)
scores('ExtraTrees', pred4, y_test)
