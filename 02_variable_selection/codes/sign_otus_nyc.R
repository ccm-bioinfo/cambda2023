...
option_list = list(
    make_option(c("-I", "--input_dir"), type = "character", default = "../selected_variables_results/significant_otus/", 
    ...
    make_option(c("-O", "--out_dir"), type = "character", default = "../selected_variables_results/nyc_otus_list/", 
...
signif_otus_best <- read.csv(
    paste0(path_to_input, prefix0, "_", prefix1, "_", prefix2, "_signif.csv")
)

# Selecting only comparisons of NYC with other cities in USA
signif_nyc_usa <- signif_otus_best %>% 
    filter(
        grepl("NYC", locs)
    ) %>% 
    filter(
        grepl("BAL", locs) | grepl("DEN", locs) | grepl("MIN", locs) |
            grepl("SAC", locs) | grepl("SAN", locs)
    )

# Get the different taxonomic levels present in the data
taxLevels <- unique(unlist(signif_nyc_usa$hlevel))

sepList <- vector("list", length = length(taxLevels))
for (i in 1:length(taxLevels)) {
    # get the taxons corresponding to a taxonomic level
    signif_temp <- signif_nyc_usa %>% 
...
signif_nyc_usa_names <- sepList[[1]]
if (length(taxLevels) > 1) {
    for (i in 2:length(taxLevels)) {
        signif_nyc_usa_names <- signif_nyc_usa_names %>% 
            full_join(sepList[[i]])
    }
}
...
signif_nyc_usa_names %>% 
    ...
    write.csv(
        file = paste0(
            path_to_output, "nyc_otus_", prefix0, "_", prefix1, "_", prefix2, 
            ".csv"
        ),
        row.names = FALSE
    )
