...
PATH_INPUT = '../../04_functional/data/metagenomic/tables/'
# PATH_OUTPUT_METACYC_CUMMULATIVE_SCALED = '../../04_functional/data/metagenomic/models/metacyc/cummulative/scaled/'
# PATH_OUTPUT_METACYC_CUMMULATIVE_UNSCALED = '../../04_functional/data/metagenomic/models/metacyc/cummulative/unscaled/'
# PATH_OUTPUT_METACYC_NONCUMMULATIVE_SCALED = '../../04_functional/data/metagenomic/models/metacyc/noncummulative/scaled/'
# PATH_OUTPUT_METACYC_NONCUMMULATIVE_UNSCALED = '../../04_functional/data/metagenomic/models/metacyc/noncummulative/unscaled/'
# PATH_OUTPUT_MIFASER = '../../04_functional/data/metagenomic/models/mifaser/'
PATH_OUTPUT = '../Outputs/'
PATH_OUTPUT_IMAGES = '../Outputs/generated_plots'
...
def model_trainning(data_name, data, out_dir):
    lab = list(data['City'])
    ...
    # Random Forest
    parameters1 = {'criterion': 'entropy', 'max_depth': 40, 'n_estimators': 50, 'random_state':0, 'n_jobs': 5}
    clf1 = RandomForestClassifier(criterion= 'entropy', max_depth= 40, n_estimators= 50, random_state=1, n_jobs= 5)
    # clf1 = GridSearchCV(rf, parameters1, verbose = 1, cv = stratified_kfold, scoring = scoring, refit = 'f1', n_jobs = 1)

    #SVC
    parameters2 = {'degree': 2, 'kernel': 'linear', 'probability': True, 'random_state':0}
    clf2 = SVC(degree = 2, kernel= 'linear', probability= True, random_state =1)
    ...
    clf3 = MLPClassifier(activation= 'relu', batch_size= 50, early_stopping= False, hidden_layer_sizes= (200,), learning_rate= 'constant', max_iter= 1000, solver= 'sgd',random_state=1)
    # clf3 = GridSearchCV(mlp, parameters3,  verbose = 1, cv = stratified_kfold, scoring = scoring, refit = 'f1', n_jobs = 1)

    #Extra trees
    parameters4 = {'criterion': 'entropy', 'max_depth': 15, 'n_estimators': 100, 'random_state':0}
    clf4 = ExtraTreesClassifier(criterion= 'entropy', max_depth= 15, n_estimators= 100, random_state=1)
    

    #KNN
    parameters5 = {'n_jobs': 1, 'n_neighbors': 2, 'weights': 'distance', 'random_state':0}
    clf5 = KNeighborsClassifier(n_jobs= 1, n_neighbors= 2, weights= 'distance')
    ...
    # clf5 = clf5.best_estimator_

    voting_classifier_hard = VotingClassifier(
        estimators=[('RF', clf1), ('SVC', clf2), ('MLP', clf3), ('ET', clf4), ('KNN', clf5)],
        voting='hard'
    )
    ...
    pred3 = clf3.predict(X_test_transformed)
    pred4 = clf4.predict(X_test_transformed)
    pred5 = clf5.predict(X_test_transformed)
...
def main():
    ...
    model_trainning('Mifaser_level_1', mifaser1, PATH_OUTPUT),
    model_trainning('Mifaser_level_2', mifaser2, PATH_OUTPUT),
    model_trainning('Mifaser_level_3', mifaser3, PATH_OUTPUT),
    model_trainning('Mifaser_level_4', mifaser4, PATH_OUTPUT),
