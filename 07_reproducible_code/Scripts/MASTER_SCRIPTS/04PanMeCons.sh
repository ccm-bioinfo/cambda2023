...
for site in ${sites[*]}; do
	...
	anvi-import-collection -c $indiv-cp-CONTIGS.db -p $indiv-$site-MERGED/PROFILE.db -C Genomes --contigs-mode $indiv-renaming-manual.tsv
	...
	sed \"s/$indiv-$site-CONTIGS.db/$indiv-cp-CONTIGS.db/g\" $indiv-$site-internal-genomes-table.txt > tmp
	#es necesario agregar este paso
	...
	anvi-pan-genome --mcl-inflation $mcl -o $indiv-$site-$mcl-PAN -g $indiv-$site-GENOMES.db -n $indiv-$site-$mcl --use-ncbi-blast -T 16
