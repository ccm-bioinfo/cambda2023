...
sites=(NYC)  # important that td is first if that will be the one onto which others are added
for site in ${sites[*]}; do
	...
	anvi-delete-misc-data -p $prefix-NYC-10-PAN/$prefix-NYC-10-PAN.db -t layers --keys-to-remove $(head -1 $prefix-$site-layers.txt | cut -f$readsStart- - | tr '\t' ',')
	...
	anvi-import-misc-data -p $prefix-NYC-10-PAN/$prefix-NYC-10-PAN.db -t items $prefix-$site-items.tmp
	anvi-import-misc-data -p $prefix-NYC-10-PAN/$prefix-NYC-10-PAN.db -t layers $prefix-$site-layers.tmp
